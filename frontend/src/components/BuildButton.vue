<!--
Author: dexter-st (https://github.com/dexter-st)
License: MIT
-->
<template>
  <div class="btn-wrapper" :class="{ 'btn-disabled': disabled && !building, 'btn-building': building }">
    <button
      class="btn"
    ...
    >
      ...
      <div class="txt-wrapper">
        <div class="txt-1" :class="{ 'txt-hide': building }">
          <span class="btn-letter" v-for="(ch, i) in idleText" :key="'idle-'+i">{{ ch }}</span>
        </div>
        <div class="txt-2" :class="{ 'txt-show': building }">
          <span class="btn-letter" v-for="(ch, i) in busyText" :key="'busy-'+i">{{ ch }}</span>
        </div>
      </div>
    </button>
  </div>
...
.btn-wrapper.btn-disabled .btn {
  opacity: 0.38;
  cursor: not-allowed;
}

.btn-wrapper.btn-building .btn {
  cursor: wait;
}
...
/* ── Text layers (absolute to overlap at same position) ── */
.txt-wrapper {
  position: relative;
  display: flex;
  align-items: center;
...
/* When building: cross-fade after 1s delay */
.txt-1.txt-hide {
  opacity: 0;
}
...
.btn-letter:nth-child(11) { animation-delay: 0.8s; }
